...
export const UPGRADE_CARDS: UpgradeCard[] = [
  ...
  C('depInjection', 'Dependency Injection', 'rare', 'Projectiles', '💉',
    '+1 projectile', 'Your weapons now receive violence from a container.', { projectiles: 1 }, 3),
  C('rubberDuck', 'Rubber Duck Insight', 'rare', 'Luck', '🦆',
    '+1 luck, +10% XP', 'You explained the problem. The duck suggested murder.', { luck: 1, xpGain: 0.1 }),
  C('garbageDay', 'Garbage Collector Tuning', 'rare', 'Damage', '🗑',
    '+22% damage', 'Aggressive collection. Generational. Personal.', { damage: 0.22 }),
  C('segShield', 'Segmentation Shield', 'rare', 'Armor', '🧱',
    '+3 armor, +20 max HP', 'SIGSEGV? Sig-blocked.', { armor: 3, maxHp: 20 }),
  C('breakpointTrap', 'Breakpoint Trap', 'rare', 'Attack Speed', '🔴',
    '+16% cooldown reduction', 'Execution pauses. Yours does not.', { cooldown: 0.16 }),
  C('profiler', 'Profiler Flamegraph', 'rare', 'Crit', '🔥',
    '+8% crit chance, +25% crit damage', 'Found the hot path. It leads through their ribs.', { critChance: 0.08, critMult: 0.25 }),
  C('observability', 'Observability Stack', 'rare', 'Pickup', '📡',
    '+45 pickup radius', 'You can now see — and yoink — everything.', { pickupRadius: 45 }),

  // ---- Epic ----
  C('compilerBlessing', 'Compiler Blessing', 'epic', 'Damage', '✨',
    '+30% damage, +8% cooldown reduction', '-O3 -funroll-bugs -march=violence', { damage: 0.3, cooldown: 0.08 }),
  C('raceCondition', 'Race Condition', 'epic', 'Attack Speed', '🏁',
    '+22% cooldown reduction, +8% move speed', 'Your attacks now arrive before they are fired.', { cooldown: 0.22, speed: 0.08 }),
  C('infiniteLoopCard', 'Infinite Loop', 'epic', 'Projectiles', '♾',
    '+2 projectiles, -5% damage', 'while(true) { fire(); } // TODO: fix later', { projectiles: 2, damage: -0.05 }, 2),
  C('mergeConflictCard', 'Merge Conflict', 'epic', 'Chaos', '⚔',
    '+40% damage, -10 max HP', 'Take both changes. Resolve nothing.', { damage: 0.4, maxHp: -10 }, 2),
  C('zeroDowntime', 'Zero-Downtime Deploy', 'epic', 'Health', '🟢',
    '+50 max HP, +1.5 HP/s regen', 'Five nines of you remain available.', { maxHp: 50, regen: 1.5 }),
