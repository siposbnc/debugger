...
- Endless mode "work hours" pun: 8-minute "Normal Work Hours", time past it = Overtime w/ richer rewards → **folded into the v0.4 P1 "Endless mode" item, reframed "Normal Work Hours & overtime"**. **Endless-only** (user clarified 2026-06-17: normal runs stay 15:00, base game loop + balance baseline unchanged) — the 8:00 workday boundary lives in Endless mode; past it = overtime with a ramping Bits/Credits multiplier and an exponential `difficulty()` term keyed off 8:00. Open at impl: reuse the 15-min `spawnPlan` + start overtime at 8:00 (leaning) vs author a compressed 8-min plan
