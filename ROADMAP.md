...
### Engine
- [~] [P1] (L) **WebGL2 world renderer** *(user 2026-06-12: late-game particle/effect density is the design — current Canvas2D renderer can't hold 60fps/<20ms under it, and density should grow beyond today's 380/900 caps)* — hand-rolled zero-dependency WebGL2 batched quad renderer for the world pass (floor/zones/entities/projectiles/effects/particles): existing procedurally-baked sprites packed into a texture atlas; per-frame `shadowBlur`/`ctx.filter` replaced by baked glows + per-quad shader tint/whiten/desat; SDF shader modes for isometric ellipses/rings/arcs; floor baked to chunk textures. New `#hud` 2D canvas takes HUD/text/damage numbers; `#banners` unchanged. Canvas2D world pass kept as automatic fallback (frozen legacy) when WebGL2 init fails. Particle cap raised well past 900 with the FPS safeguard recalibrated. Game logic / sim untouched (renderer-only)
