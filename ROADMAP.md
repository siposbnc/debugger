...
- [ ] [P2] (M) **Pause inventory redesign** — *user feedback on the v0.3 weapons panel: "feels cramped together, inventory should be main visual focus".* Make the arsenal the anchor of the pause screen: weapons as large cards (icon, level pips, resolved stat block, evolution-ready hint) in the primary pane; run stats / taken cards demoted to a secondary column or collapsible. Pure DOM/CSS in `menus.ts`, no sim impact — pairs naturally with the arsenal-tab item (shared card markup)
