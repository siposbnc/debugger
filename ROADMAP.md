...
- [ ] [P2] (S) **Weapon-change previews on level-up cards** *(from draft 2026-06-12)* — weapon-up (and new-weapon/evolution) cards show the concrete level delta the same way stat cards show "dmg 34 → 38": read the weapon's current vs next `levels[]` row and list the changed fields (damage/cooldown/count/area/pierce…), "+1 ⌁" style for count steps. Same preview surface the v0.2 stat-change/CAPPED machinery uses (`cardPreviewTest.mjs` is the harness to extend)
