...
- [x] [P1] (S) **Crunch Time** *(user 2026-06-12)* — bosses alive at 15:00 are release blockers: 30s overtime (trash descoped, spawner frozen, +50% dmg / +15% speed crunch buffs); any blocker outliving it fails the run ("RELEASE SLIPPED"). Outliving bosses no longer wins — BALANCE.md mortal-bot win targets flagged for re-certification. Sim-verified (trigger correctness, ≤15:30 resolution, clean fail path)
