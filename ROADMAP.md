...
- [x] [P1] (S) **Sec Hexa** — *done: 550 ⌬, starts Firewall; `special: 'thorns'` — contact attackers take back 20% of their contact dps (0.5s tick window per enemy so damage numbers don't spam; credited to the ally/special damage bucket). Sim: clean victory, kill-rate crossover from min 1, thorns visibly credited (3.1k over a run)*
- [x] [P1] (S) **Dana Tensor** — *done: 650 ⌬, starts Ping Storm; `special: 'xpPower'` — +1% damageMult per 100 XP collected, applied in `recompute()` and refreshed on the 1s tick (stays out of the pickup hot path). First sim exposed a real hole: Ping Storm lv 1–2 + random targeting capped the field by min 4 and starved the passive (the syntaxWand pierce lesson) — fixed weapon-side: base pierce 1, dmg 8→13, cd 1.5→1.2 (DDoS adjusted to match). Post-fix: no cap pinning (alive ≤53 pre-6:00), lv 11 @ 5:00 = fleet baseline, victories within pick-first-bot variance (ada baseline slipped 4/4 in the same batch — standing boss-TTK Backlog item, not a dana issue)*
- [x] [P1] (S) **Greybeard Cobol** — *done: 800 ⌬, starts Stack Staff; `special: 'slowImmune'` — `playerSlow` forced to 1 (voids scarab auras, marsh pools, latency fields, kernel-panic chill) with −15% speed in mods. Sim: greenfield + marsh victories (marsh is the showcase: 41 levels, 4.2k kills, Bits 2785 in band) — characters target met (8 total)*
...
- 2026-06-12 — **+3 characters shipped** (v0.3 characters target met, roster now 8): Sec Hexa (Firewall starter, 20% contact thorns → ally-damage bucket, 550 ⌬), Dana Tensor (Ping Storm starter, +1% dmg per 100 XP collected, 650 ⌬), Greybeard Cobol (Stack Staff starter, slow-immune at −15% speed, 800 ⌬). Dana's first sim caught Ping Storm's early floor failing as a starter (cap pinned by min 4 — the wand pierce lesson); fixed weapon-side (base pierce 1 + lv-1 damage/cadence buff, DDoS matched). All three sim-verified vs the watchlist; shop total now 21,925 ⌬ (BALANCE.md §4)
