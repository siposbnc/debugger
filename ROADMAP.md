...
### Easter egg (user-promoted into v0.2 — the milestone's one content exception)
- [x] [P1] (S) **"The Precipitate"** (internal id `mushi`) *(from draft 2026-06-12; personal nod, never explicit — the friend IS a chemist, NOT a developer; concept picked by user from 4 candidates, promoted from v0.3 2026-06-12 — build with the `design-entity` skill)* — premise: everything in this game is a software defect except this one entity, because he's a chemist — something little from the wet lab that wandered into prod and has no idea what a pointer is.
  - **Encounter**: ~25% of runs, once, window centered on **6:02** (±60s); a tiny goggled flask-critter *precipitates out of solution* (bubble-condense in) at the spawn ring, announced by a soft fizz SFX + a faint **gold** edge shimmer (reuse the boss-indicator machinery, no ⚠ — gold = reward, not threat). Evaporates after **23 s** (6.02×10**²³**) if unreached
  - **The mechanical joke — untargetable, not killable, and the fiction says why**: it is *purely functional — no side effects* (Haskell purity × chemical purity). Weapons pass straight through it (the debugger has no symbol for it; it's not a defect), it deals 0 contact damage, bugs ignore it; it wanders by literal **Brownian motion** (`wander` behavior: jittered random walk, no chase). You *collect* it by walking into it
  - **Reward on touch**: crystallization burst → standard chest pickup (existing `kind:'chest'`: evolution or free rare+ card) + **602 raw XP** shards + 23 Bits, with a **"DEPENDENCY RESOLVED"** banner — the *Cabal* layer (where they met = Cabal Online; Cabal = Haskell's package manager; resolving a package from a cabal is this game's native wordplay). Despawn-untouched gives nothing
  - **Despawn beat (deep cut)**: if it evaporates uncaught, it first strikes a brief, inexplicably dramatic pose wreathed in a golden battle aura — an over-the-top MMO transformation flash on a tiny flask — then pops into vapor (recognizable to anyone who ground Battle Mode in a certain Korean MMO; pure absurdism to everyone else)
  - **Body**: smallest entity in the game (radius 8 — under Cache Tick's 9): Erlenmeyer-flask body with stubby legs, oversized safety goggles, glass-white shell with teal liquid sloshing, tiny bubble trail; new sprite shape `flask` (codex thumbnail comes free)
  - **Codex**: listed last, classification "NOT A BUG": "Not a bug — wrong phylum entirely. Wandered in from the wet lab; keeps trying to titrate the memory pools. Purely functional: no side effects, which is why nothing here can touch it. Known member of a cabal of exactly two; recruitment closed years ago. Filed the littlest issue on record on its way out."
  - **Objective**: **"Cabal of Two"** — collect The Precipitate once (Bits reward); the friendship encoded as an achievement name, no names attached
  - **Sim note**: zero combat impact by construction (harmless + untargetable); only economy touch is the once-per-~4-runs chest/XP — sim-verifier after landing
  - *(Unused candidates, kept for reference: Free Radical elite, Catalyst card, Elixir pickup — the Catalyst card's "small friend from the wet lab" flavor could still ship later as a companion nod)*

  — *done 2026-06-12, exactly per spec: own entity on `Run` (never enters `enemies[]` — weapons/grid/cap/recycling/kill-stats ignore it by construction), `flask` sprite + gold `?` edge marker + fizz/resolve synth SFX, "Field sample resolved +23⌬" summary line, suspend/resume covered (optional snapshot fields, old snapshots restore clean), `dbg.mushi()` to demo it. Verified: `scripts/mushiTest.ts` 21/21 (schedule odds, ring entry, Brownian drift, evaporation, touch rewards, objective, suspend round-trips) + `scripts/mushiVisualTest.mjs` 7/7 (codex tag/thumb/copy, dbg spawn) + screenshots; build clean; sim-verifier: zero combat impact, Bits 1740 in band*
...
- 2026-06-12 — Draft batch 13: Little Mishu easter egg processed → v0.3 Cards & enemies P2 "the visiting chemist" — angle: he's a chemist, not a developer, so the entity is the one thing in the game that *isn't* a software bug; 4 concept candidates proposed (neutral critter "The Precipitate" recommended; Free Radical elite; Catalyst card; Elixir pickup), `mushi` id + Avogadro numbers as hidden layers; implementation waits on user pick (design-entity skill)
- 2026-06-12 — **The Precipitate shipped** (easter egg, user-promoted into v0.2 same day): rare untargetable wet-lab visitor (25% of runs @ ~6:02, 23s, Brownian wander, collect by touch → chest + 602 XP + 23 ⌬ + "DEPENDENCY RESOLVED"), golden battle-aura send-off when missed, codex NOT A BUG entry, "Cabal of Two" objective, `dbg.mushi()`. New mushiTest.ts (21 checks) + mushiVisualTest.mjs (7 checks); build + sim verified (no combat impact, Bits in band)
