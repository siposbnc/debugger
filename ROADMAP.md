...
- [ ] [P2] (S) **Isolated play server** *(from draft 2026-06-12: "docker support, so the game can be run in an isolated environment without interrupting Claude's work and to avoid vite's hot reload")* — proposal: solve the need without Docker. `npm run play` = `build:dev` into a separate `dist-play/` + `vite preview` on its own port — no file watching (no hot reload), and Claude's `dist/` rebuilds never touch it; the user refreshes/rebuilds only when they choose. Docker itself adds a Docker Desktop dependency without extra isolation for a static bundle — revisit only if environment *parity* (clean-browser/OS repro) ever becomes the actual need. Note: `npm run deploy:test` (2026-06-12) already covers the "play a frozen snapshot" case remotely; this item is the local-latest variant
