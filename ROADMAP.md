...
- [x] [P1] (L) **Prestige core** *(done 2026-07-02 — PRESTIGE.md §8 stage 1)*: save fields (`rewrites`/`legacyTokens`/`tree`/`cycleBits`/`cycleOvertimeBest`/`keptMeta`), `data/prestige.ts` (token formula `3×maps + √(cycleBits/150) + overtimeMin/3`), SHIP IT button + Rewrite screen (live token counter, reset/persist table, arm/confirm, cycle summary), the reset itself (`save/prestige.ts shipRewrite`), Ship Bonus (+30% Bits line, +10% XP per rewrite), `you: vN.0` menu version, `prestigeTest.ts` (28 checks)
- [x] [P1] (L) **Prestige tree** *(done 2026-07-04 — PRESTIGE.md §8 stages 2–3)*: token-spend tree UI (3-branch Legacy Tree screen) + passive nodes (Warm Boot, Severance Package, Persistent Config w/ SHIP-IT picker, Preflight Check, Compound Interest, Continuous Learning, Time Dilation) + actives (Dash Space/pad-B with i-frames + HUD pips, Restore Point revives, Sudo Mode Shift/pad-Y); `prestigeTest.ts` 67 checks, baseline sim clean
...
- 2026-07-04 — **Prestige tree shipped (stages 2+3 of 4)**: `data/prestige.ts` grows the 10-node/3-branch tree (`treePerks()` resolves ranks into the one `PrestigePerks` shape game code sees; sims stay neutral by default). Stage 2 passives: Warm Boot (start lv 2–4 + owed picks), Severance (500/1.5k/3k start ⌬), Persistent Config (kept-meta picker on the Rewrite screen, slot-clamped in `shipRewrite`), Preflight Check (evolution-ready at lv 7), Compound Interest (+3%⌬/rank ∞), Continuous Learning (+3% XP/rank ∞), Time Dilation (1.25×/1.5× settings toggle) + Legacy Tree spend screen off the main menu. Stage 3 actives: Dash (Space/pad-B, 140u burst, i-frames, HUD cooldown pip), Restore Point (revive @50% HP + magnet burst, pause-sheet counter), Sudo Mode (Shift/pad-Y, invuln +50% dmg +25% CDR); binds remappable once owned, cooldowns/charges survive suspend. prestigeTest 67 checks; baseline sim verified untouched; E2E click-through (buy → warm-boot picks → dash 143u → sudo pip). Next: stage 4 endgame levers (P2, balance-flagged): token formula + live counter (first rewrite ≈ 26 ⟲ on the calibrated profile), SHIP IT → Rewrite screen (breakdown, reset/persist table, KILL-PROCESS-style arm/confirm, suspended-run block) → v(N+1).0 receipt; full reset per §4 with `keptMeta` hook ready for Persistent Config; Ship Bonus +30% Bits (breakdown line) & +10% XP per rewrite wired through Run + suspend/resume; pre-prestige saves grandfather their lifetime bits/runs/overtime into cycle 1. prestigeTest 28 checks + E2E click-through; single click only ARMS (verified). Next: stage 2, tree UI + passive nodes
