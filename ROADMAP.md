...
- [ ] [P1] (L) **Endless mode — "Normal Work Hours" & overtime** — *(refined from draft 2026-06-13, reframed 2026-06-17)* the work-hours pun reshapes the whole loop:
  - **Base run shortens 15:00 → 8:00** *(draft 2026-06-17: an 8-hour workday → 8 minutes)*. This is **"Normal Work Hours"** — the new victory time for **every** run, not just Endless. ⚠️ **Scope is game-wide, not Endless-only**: it changes the default experience and **invalidates the current balance baseline** — every map's `spawnPlan` phase table compresses (boss cadence + the 12:00 finale move earlier; propose proportional compression so the finale lands at ~8:00, but the exact boss times are a per-map retune, not a uniform ×8⁄15), and BALANCE.md's absolute-minute targets all shift (see watchlist note). Full sim re-cert. **Do this together with the rarity-gating + per-map-variants balance rewrite** — all three re-anchor the early game, so it's one BALANCE.md rewrite, not three.
  - **Endless = clock past 8:00 is "Overtime"** *(hints richer rewards — overtime should pay better, draft)*. **Per-map unlock**: clearing a map (the 8:00 victory) unlocks Endless *for that map only* — pre-run toggle on its map-select card, hidden until earned. Needs the `mapVictories` per-map win counter (shared with the gated-map-progression item below).
  - In overtime: the 8:00 victory + Bits payout **bank immediately** (an overtime death never forfeits the win — otherwise nobody opts in twice); boss order reshuffles from the map's `bossPool`, the unique finale re-enters the pool at elevated `bossTier`; `difficulty()` gains an **exponential overtime term** (draft: "exponential scaling after [normal hours]" — e.g. hp/damage ×~1.06–1.08 per overtime minute, tuned so even god-tier builds collapse a fixed span past 8:00: every endless run has a definite end, which is what makes best-time-per-map a real leaderboard stat in the codex). Overtime's "more generous rewards" framing = a Bits/Credits multiplier that ramps with the minute, paying for the rising risk
...
**Run length is also dropping 15:00 → 8:00 ("Normal Work Hours", Endless item): the
absolute-minute targets below (min-6 kill-rate crossover, 60–100s first-boss TTK) are
calibrated for a 15-minute run and must be re-derived against the 8:00 victory when that
lands — fold it into the same single BALANCE.md rewrite as the two items above.**
...
- 2026-06-17 — Draft batch 25 processed: **Endless item reframed as "Normal Work Hours & overtime"** (draft work-hours pun) — base run shortens 15:00 → 8:00 game-wide (flagged: rewrites every map's `spawnPlan` + folds into the one rarity-gating/variants BALANCE.md rewrite; watchlist gains the run-length note), Endless = clock past 8:00 with a ramping overtime reward multiplier. **Dev tooling +1 (P3 S)**: `dbg.boss`/`dbg.remove`/`dbg.spawn` hooks (the "spawn specific event" ask already ships as `dbg.event`)
