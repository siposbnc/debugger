...
- [ ] [P2] (S) **Easter egg: "The Precipitate"** (internal id `mushi`) *(from draft 2026-06-12; personal nod, never explicit — the friend IS a chemist, NOT a developer; concept picked by user 2026-06-12 from 4 candidates, refined spec below — build with the `design-entity` skill)* — premise: everything in this game is a software defect except this one entity, because he's a chemist — something little from the wet lab that wandered into prod and has no idea what a pointer is.
  ...
  - **The mechanical joke — untargetable, not killable**: weapons pass straight through it (the debugger has no symbol for it; it's not a software defect). Bugs ignore it; it ignores everything, wandering by literal **Brownian motion** (`wander` behavior: jittered random walk, no chase). Contact damage 0. You *collect* it by walking into it
  - **Reward on touch**: crystallization burst → standard chest pickup (existing `kind:'chest'`: evolution or free rare+ card) + **602 raw XP** shards + 23 Bits; despawn-untouched gives nothing
  ...
  - **Codex**: listed last, classification "NOT A BUG": "Not a bug — wrong phylum entirely. Wandered in from the wet lab; keeps trying to titrate the memory pools. Asked what a pointer was; politely declined to learn. Filed the littlest issue on record on its way out."
  - **Objective**: "Field Work" — collect The Precipitate once (Bits reward)
