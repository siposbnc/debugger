...
- [ ] [P2] (M) **In-run currency: API Tokens** — dropped by in-run events and elites; spent during the run at a **Package Registry** terminal (spawns after each boss, marked like events) on one-run consumables: instant heal, magnet sweep, +1 reroll/banish, short buffs. Separate economy from Bits (meta) and XP (leveling) — unspent tokens die with the run. Depends on in-run events; keep the item pool small (4–6) and data-driven. *Ships with two meta upgrades (draft 2026-06-13): elite **token drop rate** and **token drop amount** — no matching card stat exists, so their progressive-unlock trigger is the first token ever picked up (mark via the `spawnedKinds`-style discovery path), not a card pick*
