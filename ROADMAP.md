...
- [ ] [P2] (S) **Easter egg: the visiting chemist** *(from draft 2026-06-12; personal nod, never explicit — user clarified the draft's flipped note: the friend IS a chemist, NOT a developer)* — premise that carries the joke: everything in this game is a software defect, except this one entity, because he's a chemist — something little from the wet lab that wandered into prod and has no idea what a pointer is. Concept candidates (user picks before implementation; build with the `design-entity` skill):
  1. **The Precipitate** *(recommended)* — rare neutral critter: precipitates out of solution mid-run (low chance per run, min 4–10), moves by literal Brownian motion, ignores the player entirely, evaporates after ~25s; caught before it leaves → Sample Flask chest + 602 XP ("a decimole of experience"). Codex: "Not a bug — wrong phylum entirely. Wandered in from the wet lab; keeps trying to titrate the memory pools. Filed the littlest issue on record on its way out."
  2. **Free Radical** — rare elite enemy: fast, erratic, unpaired; bonds with the first bug it touches (mutual buff — "management mistook this for pair programming"); on death sets off a chain-reaction damage cascade through nearby enemies
  3. **Catalyst** — rare stat card (−cooldown): "lowers the activation energy of everything you ship"; flavor credits a small friend from the wet lab who maintains a compiler is just a very slow Bunsen burner (cheapest option: pure data)
  4. **Elixir** — rare flask ground pickup: full heal + short buff; Elixir-the-language vs the-thing-an-actual-chemist-brews ("left behind by someone who knows what the glassware is for")

  Hidden layers wherever it lands: internal entity id `mushi` (anagram of "Mishu"; Japanese for *bug* — reserved for the one entity that isn't one), Avogadro numbers in flavor/drops (6.02×10²³, 602), "the littlest issue" phrasing. The friend recognizes himself by being the tiny chemist in a developer's world; the name itself never appears
